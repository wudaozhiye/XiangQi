...
public class PopUpManager : MonoBehaviour
{
    ...
    private static void OnNewComponentObject(Data obj)
    {
        ...
        obj.mask_gameobject =
            GameObject.Instantiate(UnityEngine.Resources.Load(NIREUS_UI_MASK_PATH, typeof(GameObject)) as GameObject);
    }
    ...
    public void AddPopUp(UIDialog tpl, bool needsWaiting = false, object param = null, bool show_mask = true,
        bool click_mask_close = true, float mask_alpha = 0.7f, Func<bool> clickFunc = null)
    {
        if (tpl == null) return;
    ...
    private void _showDialog(Data data)
    {
        ...
        data.depth_ui_root.transform.position = new Vector3((_displayingList.Count + 100) * 20, 0, 0);
        InitMask(data);
...
// #endif
    ...
    int Find(UIDialog tpl)
    {
        if (tpl == null)
        {
            Debug.LogError("UIDialog tpl is null");
            return -1;
        }
    ...
    public bool IsShow(UIDialog tpl)
    {
        return Find(tpl) >= 0;
    }
